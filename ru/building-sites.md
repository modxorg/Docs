---
title: "Администрирование вашего сайта"
translation: "building-sites"
---

Этот раздел содержит информацию о поддержке и администрировании сайта MODX.

- [Настройка](_legacy/administering-your-site/settings)
- [Использование дружественных URL](getting-started/friendly-urls)
- [Контексты](building-sites/contexts)
- [Настройка панели администрирования](building-sites/client-proofing/form-customization)
- [Безопасность](building-sites/client-proofing/security)
