...
Фильтр вывода применяет один или несколько выходных модификаторов, встроенных в ядро. Синтаксис выглядит так:
...
| strtotime                                | Преобразует строку даты в метку времени Unix. Полезно сочетать это с выходным фильтром даты. Похож на PHP [strtotime](http://www.php.net/strtotime). Принимает дату на вход. Посмотрите [Форматы даты](building-sites/tag-syntax/date-formats "Форматы даты").                             | `[[+thetime:strtotime]]`                           |
