...
Начиная с FormIt 1.4.0, изменился способ валидации. Старый метод валидации через имена полей формы будет работать до FormIt 2.0.0, после чего будет удалён. Рекомендуется использовать новый способ через свойство `&validate`.
