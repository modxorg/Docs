...
Валидация (см. [Валидаторы FormIt](extras/formit/formit.validators)) в этом примере снимает теги с сообщения, проверяет email и требует заполнения всех полей через `&validate`.

Форма также использует [reCAPTCHA v3](https://www.google.com/recaptcha/about/) через хук [recaptcha](extras/formit/formit.hooks/recaptcha) (FormIt 5.2.1+). В системных настройках задайте:

- `formit.recaptcha_site_key`
- `formit.recaptcha_secret_key`
- при необходимости `formit.recaptcha_min_score` (по умолчанию `0.5`)
- `formit.frontend_js` = `js/web/formit.js` (нужен для запроса токена)
...
> Убедитесь, что `emailFrom` установлен в `[[++emailsender]]`, иначе будет использоваться email из поля формы. Большинство хостингов отклоняют письма с адресом отправителя с чужого домена.
