...
FormIt - это динамический [Сниппет](developing-in-modx/basic-development/snippets "Сниппет") обработки форм для MODX Revolution. Он обрабатывает форму после отправки, выполняя проверку и последующие действия, такие как отправка электронного письма. Он не создает форму, но может повторно заполнить ее, если проверка не была пройдена.
...
Formit можно загрузить из Менеджера MODX Revolution через [Управление пакетами](developing-in-modx/advanced-development/package-management) или из репозитория MODX здесь: <https://modx.com/extras/package/formit>.
...
Актуальная линейка пакета: **FormIt 5.2.x** ([Sterc/FormIt](https://github.com/Sterc/FormIt), ветка `3.x`). Кратко по релизам, которые чаще всего ломают старые инструкции:

- **5.2.1+**: хук `recaptcha` переведён на **reCAPTCHA v3**. Нужны `formit.recaptcha_site_key` / `formit.recaptcha_secret_key` / `formit.recaptcha_min_score` и фронтенд JS (`formit.frontend_js`). Старые `recaptcha_public_key` / `recaptcha_private_key` и свойства `recaptchaTheme`, `recaptchaJs`, `recaptcha_use_ssl` больше не используются. См. [recaptcha](extras/formit/formit.hooks/recaptcha).
- **5.2.0+**: [отправка через AJAX](extras/formit/formit.ajax) через `formit.js` и `[[!+fi.ajaxToken]]`.
- **5.1.2+**: загрузки файлов режутся по `formit.upload_blocked_extensions` (опасные расширения блокируются даже если их разрешает MODX).
- **5.0+**: пакет рассчитан на MODX 3.
- **3.0**: шифрование сохранённых форм с `mcrypt` на `openssl` (в менеджере есть страница миграции).

Начиная с FormIt 2.2.9 ко всем полям автоматически применяется `html_entities`. Чтобы сохранить HTML-теги, используйте валидатор `allowSpecialChars` на нужных полях.

Начиная с FormIt 1.1.4 ко всем полям автоматически применяется `stripTags`. Чтобы сохранить HTML-теги, используйте `allowTags` и перечислите разрешённые теги.
...
| trimValuesBeforeValidation | Обрезать пробелы у строковых значений перед валидацией.                                                                                                                                                                                                                                                                                                                                                                          | 1                                                                                |
| saveTmpFiles              | Сохранять загруженные файлы во временное хранилище между шагами формы.                                                                                                                                                                                                                                                                                                                                                           | 0                                                                                |
| storeAttachments          | Связано с [FormItSaveForm](extras/formit/formit.hooks/formitsaveform): сохранять вложения вместе с формой.                                                                                                                                                                                                                                                                                                                        | 0                                                                                |
| recaptchaAction           | Имя action для reCAPTCHA v3 (см. [recaptcha](extras/formit/formit.hooks/recaptcha)).                                                                                                                                                                                                                                                                                                                                              | `submit`                                                                         |
