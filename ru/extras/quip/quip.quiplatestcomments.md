...
description: "Вспомогательный сниппет Quip для вывода последних комментариев по ветке, пользователю или сайту"
translation: "extras/quip/quip.quiplatestcomments"
---

## Сниппет QuipLatestComments

QuipLatestComments это вспомогательный сниппет Quip. Он быстро показывает последние комментарии по ветке, пользователю или по всему сайту.
...
Последние комментарии в ветке:

```php
[[QuipLatestComments? &type=`thread` &thread=`mythread`]]
```

Последние комментарии пользователя с username `jb2009`:
...
| tpl       | Чанк для каждого комментария.                                                                 | quipLatestComment       |
