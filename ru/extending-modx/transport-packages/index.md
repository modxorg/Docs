...
`xPDOObjectVehicle` или контейнеры с базой данных часто имеют следующие дополнительные ключи. В скрипте сборки их задают константами `xPDOTransport::*`. Подробности в [Создании скрипта сборки](extending-modx/transport-packages/build-script):

- **preserve\_keys** - если `true`, значения первичного ключа из пакета подставляются при загрузке объекта. Нужно для натуральных или строковых ключей (например, `key` у Context). Для обычных autoincrement ID оставляйте `false`, чтобы БД выдала новый ID.
- **update\_object** - если `true`, контейнер обновляет объект, когда совпадающая строка уже есть. Если `false`, строка пропускается. Удаление пакета тоже зависит от этого флага: xPDO удаляет объект только когда обновление разрешено (и **uninstall_object** не равен `false`).
- **uninstall\_object** - если `false`, объект остаётся в БД при удалении пакета. Если ключ не задан, по умолчанию `true`. Для удаления строки всё равно нужен **update_object** = `true`.
- **unique\_key** - имя столбца, по которому объект однозначно ищут. Часто это не первичный ключ: autoincrement-поля в разных БД обычно не совпадают.
- **preexisting\_mode** - как обращаться с объектами, которые уже есть на сайте (`preserve_preexisting`, `remove_preexisting` или `restore_preexisting`). Можно задать на контейнере или передать при install/uninstall. Подробности на странице скрипта сборки.
- **abort\_install\_on\_vehicle\_fail** - если `true` и установка этого контейнера падает, установка пакета останавливается.
- **resolve\_files** / **resolve\_files\_remove** / **resolve\_php** - включают или отключают file- и PHP-резолверы и решают, удалять ли цели файловых резолверов при uninstall.
- **install\_files** / **uninstall\_files** - для `xPDOFileVehicle`: не копировать файлы при установке или оставить их при uninstall.
- **related\_objects** - массив связанных объектов к основному объекту контейнера. Иногда без «связанных» объектов нужный результат не собрать. Пример: упаковать Сниппеты внутри Категории. Объект контейнера: Категория. Связанные объекты: Сниппеты.
- **related\_object\_attributes** - атрибуты для связанных объектов выше.
- **namespace** - как в манифесте: значение группировки объектов в `ТП`.
